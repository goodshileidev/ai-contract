...
> **主力方案**: Elasticsearch 8.x+ (kNN向量检索)
> **备选方案**: Pinecone云服务
...
### 2. Elasticsearch向量存储服务（主力方案）

```python
# app/services/ai/elasticsearch_store.py
# Python AI服务 - Elasticsearch向量存储（主力）
...
class ElasticsearchVectorStore:
    ...
    def __init__(self):
        ...
        # 使用LlamaIndex的ElasticsearchStore
...
### 3. Pinecone向量存储服务（备选方案）

```python
# app/services/ai/pinecone_store.py
# Python AI服务 - Pinecone向量存储（备选）
from typing import List, Dict, Any, Optional
import pinecone
from llama_index.vector_stores import PineconeVectorStore

class PineconeVectorStore:
    """Pinecone向量存储服务（云端备选方案）"""
    ...
    def __init__(self):
        ...
        # 使用LlamaIndex的PineconeVectorStore
...
class KnowledgeGraph:
    ...
    async def create_entity(
        ...
        Args:
            entity_type: 实体类型（Requirement, Capability, Project, Person, Case）
            properties: 实体属性
...
class BidDocumentWorkflow:
    ...
    async def analyze_requirements(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        response = await self.llm_client.chat(
            messages=[
                {"role": "system", "content": "你是招投标需求分析专家。"},
                {"role": "user", "content": prompt}
            ]
        )
    ...
    async def generate_technical_solution(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        response = await self.llm_client.chat(
            messages=[
                {"role": "system", "content": "你是技术方案撰写专家。"},
                {"role": "user", "content": prompt}
            ],
    ...
    def __init__(self):
        ...
        self.assistants = {
            AssistantType.REQUIREMENT_ANALYST: {
                "model": "gpt-4-turbo-preview",
                "temperature": 0.2,
                "system_prompt": "你是需求分析专家，擅长从招标文件中精准识别需求。"
            },
            AssistantType.SOLUTION_WRITER: {
                "model": "gpt-4-turbo-preview",
                "temperature": 0.7,
                "system_prompt": "你是技术方案撰写专家，擅长撰写专业的技术方案。"
            },
            AssistantType.COMMERCIAL_ASSISTANT: {
                "model": "gpt-3.5-turbo",
                "temperature": 0.5,
                "system_prompt": "你是商务方案专家，擅长商务条款和报价策略。"
            },
            AssistantType.COMPLIANCE_REVIEWER: {
                "model": "gpt-4-turbo-preview",
                "temperature": 0.1,
                "system_prompt": "你是合规审核专家，擅长检查标书的合规性。"
            },
            AssistantType.QUALITY_ASSURER: {
                "model": "gpt-4-turbo-preview",
                "temperature": 0.3,
                "system_prompt": "你是质量保证专家，擅长检查内容质量和完整性。"
            }
        }
...
| 日期 | 版本 | 修改者 | 修改内容概要 |
|------|------|--------|-------------|
| 2025-11-26 | 1.1 | claude-sonnet-4-5 (claude-sonnet-4-5-20250929) | 添加元信息头部；更新为Python FastAPI服务；LlamaIndex为主力框架；Elasticsearch为主力向量存储；明确服务独立部署 |
...
**创建时间**: 2025年11月15日
**最后更新**: 2025年11月26日
**服务**: Python FastAPI (独立部署)
**端口**: 8001
**主力技术栈**: FastAPI + LlamaIndex + Elasticsearch
**备用技术栈**: LangChain + Pinecone

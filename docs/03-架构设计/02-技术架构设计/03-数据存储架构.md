---
文档类型: 架构文档
需求编号: DOC-2025-11-001
创建日期: 2025-11-29
创建者: claude-sonnet-4-5 (claude-sonnet-4-5-20250929)
最后更新: 2025-11-29
更新者: claude-sonnet-4-5 (claude-sonnet-4-5-20250929)
状态: 已批准
---

# AI标书智能创作平台 - 数据存储架构

> **关系数据库**: PostgreSQL 14+ (主数据存储)
> **缓存**: Redis 7+ (会话、缓存、队列)
> **向量数据库**: Elasticsearch 8+ (向量检索和全文搜索)
> **消息队列**: RabbitMQ + Celery (异步任务处理)

## 📋 文档导航

本文档是技术架构设计的一部分,其他相关文档:

1. [00-技术架构总览.md](./00-技术架构总览.md) - 整体架构和前端技术架构
2. [01-后端服务架构.md](./01-后端服务架构.md) - Java和Python服务架构详解
3. [02-AI能力架构概述.md](./02-AI能力架构概述.md) - AI服务架构设计概述
4. **03-数据存储架构.md**(本文档)- 数据存储和消息队列架构
5. [04-部署运维架构.md](./04-部署运维架构.md) - 部署、安全和监控架构

## 💾 数据存储架构

### 数据库设计
```sql
-- 核心数据表结构
-- 用户表
CREATE TABLE users (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    username VARCHAR(50) UNIQUE NOT NULL,
    email VARCHAR(100) UNIQUE NOT NULL,
    password_hash VARCHAR(255) NOT NULL,
    company_id UUID REFERENCES companies(id),
    role VARCHAR(20) NOT NULL DEFAULT 'user',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- 项目表
CREATE TABLE projects (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name VARCHAR(200) NOT NULL,
    description TEXT,
    tender_document_url VARCHAR(500),
    status VARCHAR(20) DEFAULT 'draft',
    deadline TIMESTAMP,
    budget DECIMAL(15,2),
    created_by UUID REFERENCES users(id),
    company_id UUID REFERENCES companies(id),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- 模板表
CREATE TABLE templates (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name VARCHAR(200) NOT NULL,
    description TEXT,
    category VARCHAR(50) NOT NULL,
    industry VARCHAR(50),
    content JSONB NOT NULL,
    variables JSONB,
    is_public BOOLEAN DEFAULT false,
    created_by UUID REFERENCES users(id),
    company_id UUID REFERENCES companies(id),
    version INTEGER DEFAULT 1,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- 企业能力库表
CREATE TABLE enterprise_capabilities (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    company_id UUID REFERENCES companies(id),
    type VARCHAR(20) NOT NULL, -- 'product', 'service', 'project', 'personnel'
    name VARCHAR(200) NOT NULL,
    description TEXT,
    metadata JSONB,
    -- 注意：向量存储在Elasticsearch中
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- 标书文档表
CREATE TABLE bid_documents (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    project_id UUID REFERENCES projects(id),
    template_id UUID REFERENCES templates(id),
    title VARCHAR(200) NOT NULL,
    content JSONB NOT NULL,
    status VARCHAR(20) DEFAULT 'draft',
    quality_score DECIMAL(5,2),
    ai_generated BOOLEAN DEFAULT false,
    created_by UUID REFERENCES users(id),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
```

### 缓存策略
```typescript
// Redis缓存设计
interface CacheStrategy {
  // 会话缓存
  session: {
    key: "session:{user_id}";
    ttl: 86400;  // 24小时
    data: UserSession;
  };

  // 项目数据缓存
  project: {
    key: "project:{project_id}";
    ttl: 3600;   // 1小时
    data: Project;
  };

  // 模板缓存
  template: {
    key: "template:{template_id}";
    ttl: 7200;   // 2小时
    data: Template;
  };

  // AI生成结果缓存
  ai_generation: {
    key: "ai_gen:{hash}";
    ttl: 604800; // 7天
    data: AIGenerationResult;
  };

  // 向量检索缓存
  vector_search: {
    key: "vector_search:{query_hash}";
    ttl: 1800;   // 30分钟
    data: SearchResult[];
  };
}
```

## 🔄 消息队列架构

### 异步任务处理
```python
# Celery任务队列设计
from celery import Celery

celery_app = Celery('aibidcomposer')

# 任务定义
@celery_app.task
def analyze_tender_document(document_id: str):
    """分析招标文档（调用Python AI服务）"""
    pass

@celery_app.task
def generate_bid_content(project_id: str, template_id: str):
    """生成标书内容（调用Python AI服务）"""
    pass

@celery_app.task
def check_document_quality(document_id: str):
    """检查文档质量（调用Python AI服务）"""
    pass

@celery_app.task
def export_document(document_id: str, format: str):
    """导出文档（Java服务处理）"""
    pass

# 工作流编排
@celery_app.task
def bid_generation_workflow(project_id: str):
    """标书生成工作流"""
    # 1. 分析招标文件
    analysis = analyze_tender_document.delay(project_id)

    # 2. 等待分析完成
    analysis.get()

    # 3. 生成标书内容
    generation = generate_bid_content.delay(project_id, template_id)

    # 4. 等待生成完成
    generation.get()

    # 5. 质量检查
    quality_check = check_document_quality.delay(document_id)

    return quality_check.get()
```

## 🔗 相关文档

- **技术架构总览**: [00-技术架构总览.md](./00-技术架构总览.md)
- **后端服务架构**: [01-后端服务架构.md](./01-后端服务架构.md)
- **AI能力架构**: [02-AI能力架构概述.md](./02-AI能力架构概述.md)
- **数据库设计详解**: [@docs/03-架构设计/02-数据库设计.md](../02-数据库设计.md)
- **部署运维架构**: [04-部署运维架构.md](./04-部署运维架构.md)

## 修改历史

| 日期 | 版本 | 修改者 | 修改内容概要 |
|------|------|--------|-------------|
| 2025-11-29 | 1.0 | claude-sonnet-4-5 (claude-sonnet-4-5-20250929) | 从02-技术架构设计.md拆分创建数据存储架构文档 |

---

**文档版本**: v1.0
**创建时间**: 2025年11月29日
**文档状态**: ✅ 已批准
